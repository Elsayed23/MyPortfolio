import { faLinkedin, faWhatsapp, faGithub, faHackerrank } from '@fortawesome/free-brands-svg-icons';
import API from '../Images/API.svg'

export const navLinks = [
    {
        id: 1,
        url: '/',
        title: 'home'
    },
    {
        id: 2,
        url: '/about',
        title: 'about'
    },
    {
        id: 3,
        url: '/projects',
        title: 'projects'
    },
    {
        id: 4,
        url: '/contact',
        title: 'contact'
    },
]

export const socialData = [
    {
        id: 1,
        icon: faLinkedin,
        url: 'https://www.linkedin.com/in/el-sayed-kewan-48565a26b/'
    },
    {
        id: 2,
        icon: faHackerrank,
        url: 'https://www.hackerrank.com/profile/elsayedkewan123'
    },
    {
        id: 3,
        icon: faGithub,
        url: 'https://github.com/Elsayed23'
    },
    {
        id: 4,
        icon: faWhatsapp,
        url: 'https://wa.me/+201040578478'
    },
]

export const projectsData = [
    {
        thumbnailImage: require('../Images/kemet.png'),
        title: 'Travel Company',
        demo: 'https://elsayed23.github.io/Kemettraveluae/',
        code: 'https://github.com/Elsayed23/Kemettraveluae'
    },
    {
        thumbnailImage: require('../Images/quran.png'),
        title: 'َQuran App',
        demo: 'https://elsayed23.github.io/quran/',
        code: 'https://github.com/Elsayed23/quran'
    },
    {
        thumbnailImage: require('../Images/fullStackE-commerce.png'),
        title: 'Full Stack e-commerce',
        demo: 'https://elsayed-full-stack-ecommerce.vercel.app/',
        code: 'https://github.com/Elsayed23/fullstack-e_commerce'
    },

    {
        thumbnailImage: require('../Images/pokemon.png'),
        title: 'Pokemons Wallet',
        demo: 'https://elsayed23.github.io/pokemons-wallet/',
        code: 'https://github.com/Elsayed23/pokemons-wallet'
    },
    {
        thumbnailImage: require('../Images/nike.png'),
        title: 'Nike',
        demo: 'https://elsayed23.github.io/nike/',
        code: 'https://github.com/Elsayed23/nike'
    },
    {
        thumbnailImage: require('../Images/restoran.png'),
        title: 'Restoran',
        demo: 'https://elsayed23.github.io/Restoran/',
        code: 'https://github.com/Elsayed23/Restoran'
    },
    {
        thumbnailImage: require('../Images/todo-app.png'),
        title: 'Todo App',
        demo: 'https://elsayed23.github.io/todo-app/',
        code: 'https://github.com/Elsayed23/todo-app'
    },
    {
        thumbnailImage: require('../Images/GPT-3.png'),
        title: 'GPT-3_jsm',
        demo: 'https://elsayed23.github.io/GPT-3/',
        code: 'https://github.com/Elsayed23/GPT-3'
    },
    {
        thumbnailImage: require('../Images/local_face.png'),
        title: 'Local Face store',
        demo: 'https://elsayed23.github.io/local_face_store/',
        code: 'https://github.com/Elsayed23/local_face_store'
    },
    {
        thumbnailImage: require('../Images/recipes.png'),
        title: 'Recipes App',
        demo: 'https://elsayed23.github.io/RecipesApp/',
        code: 'https://github.com/Elsayed23/RecipesApp'
    },
    {
        thumbnailImage: require('../Images/qrcode-generator.png'),
        title: 'QR-Code Generator',
        demo: 'https://elsayed23.github.io/qr-codeGenerator/',
        code: 'https://github.com/Elsayed23/qr-codeGenerator'
    },
    {
        thumbnailImage: require('../Images/fresh-cart.png'),
        title: 'Fresh Cart (e-commerce)',
        demo: 'https://elsayed23.github.io/e-commerce-freshCart/',
        code: 'https://github.com/Elsayed23/e-commerce-freshCart'
    },
    {
        thumbnailImage: require('../Images/doml.png'),
        title: 'Doml',
        demo: 'https://elsayed23.github.io/doml/',
        code: 'https://github.com/Elsayed23/doml'
    },
    {
        thumbnailImage: require('../Images/ocean.png'),
        title: 'Ocean',
        demo: 'https://ocean-next-js.vercel.app/',
        code: 'https://github.com/Elsayed23/ocean_Next.js'
    },
    {
        thumbnailImage: require('../Images/flight_booking.png'),
        title: 'Flight Booking',
        demo: 'https://elsayed23.github.io/flight_booking/',
        code: 'https://github.com/Elsayed23/flight_booking'
    },
    {
        thumbnailImage: require('../Images/NFT-market.png'),
        title: 'NFT-marketplace',
        demo: 'https://elsayed23.github.io/NFT-Marketplace/',
        code: 'https://github.com/Elsayed23/NFT-Marketplace'
    },
    {
        thumbnailImage: require('../Images/hosting.png'),
        title: 'eco-Hosting',
        demo: 'https://elsayed23.github.io/eco-hosting/',
        code: 'https://github.com/Elsayed23/eco-hosting'
    },
    {
        thumbnailImage: require('../Images/steel.png'),
        title: 'Steel',
        demo: 'https://elsayed23.github.io/steel-company/',
        code: 'https://github.com/Elsayed23/steel-company'
    },
    {
        thumbnailImage: require('../Images/gym.png'),
        title: 'GYMSTER (Gym)',
        demo: 'https://elsayed23.github.io/Gym/',
        code: 'https://github.com/Elsayed23/Gym'
    },
    {
        thumbnailImage: require('../Images/coffee.png'),
        title: 'Coffee',
        demo: 'https://elsayed23.github.io/coffee/',
        code: 'https://github.com/Elsayed23/coffee'
    },
    {
        thumbnailImage: require('../Images/twitter-clone.png'),
        title: 'Twitter clone',
        demo: 'https://elsayed23.github.io/twitter-clone/',
        code: 'https://github.com/Elsayed23/twitter-clone'
    },
    {
        thumbnailImage: require('../Images/lugx.png'),
        title: 'Lugx',
        demo: 'https://elsayed23.github.io/lugx/',
        code: 'https://github.com/Elsayed23/lugx'
    },
    {
        thumbnailImage: require('../Images/sign-up.png'),
        title: 'SignUp page',
        demo: 'https://elsayed23.github.io/sign-up_page/',
        code: 'https://github.com/Elsayed23/sign-up_page'
    },
    {
        thumbnailImage: require('../Images/log-in.png'),
        title: 'LogIn page',
        demo: 'https://elsayed23.github.io/log-in-page/',
        code: 'https://github.com/Elsayed23/log-in-page'
    },
    {
        thumbnailImage: require('../Images/huddle.png'),
        title: 'Huddle',
        demo: 'https://elsayed23.github.io/huddle-landing/',
        code: 'https://github.com/Elsayed23/huddle-landing'
    },
    {
        thumbnailImage: require('../Images/fylo.png'),
        title: 'Fylo',
        demo: 'https://elsayed23.github.io/fylo-landing/',
        code: 'https://github.com/Elsayed23/fylo-landing'
    },
    {
        thumbnailImage: require('../Images/clipboard.png'),
        title: 'Clipboard',
        demo: 'https://elsayed23.github.io/landing-p-a-g-e/',
        code: 'https://github.com/Elsayed23/landing-p-a-g-e'
    },
    {
        thumbnailImage: require('../Images/spec-page.png'),
        title: 'Special page',
        demo: 'https://elsayed23.github.io/Spec-page/',
        code: 'https://github.com/Elsayed23/Spec-page'
    },
    {
        thumbnailImage: require('../Images/memeGenerator.png'),
        title: 'Meme Generator',
        demo: 'https://elsayed23.github.io/MemeGenerator/',
        code: 'https://github.com/Elsayed23/MemeGenerator'
    },
    {
        thumbnailImage: require('../Images/spec-design.png'),
        title: 'Special design',
        demo: 'https://elsayed23.github.io/special-design/',
        code: 'https://github.com/Elsayed23/special-design'
    },
    {
        thumbnailImage: require('../Images/bondi.png'),
        title: 'Bondi',
        demo: 'https://elsayed23.github.io/Bondi/',
        code: 'https://github.com/Elsayed23/Bondi'
    },
    {
        thumbnailImage: require('../Images/leon.png'),
        title: 'Leon',
        demo: 'https://elsayed23.github.io/leon/',
        code: 'https://github.com/Elsayed23/leon'
    },
    {
        thumbnailImage: require('../Images/multi-step.png'),
        title: 'Multi step (front-end-mentor)',
        demo: 'https://elsayed23.github.io/multi-step/',
        code: 'https://github.com/Elsayed23/multi-step'
    },
    {
        thumbnailImage: require('../Images/calculator.png'),
        title: 'Calculator App (front-end-mentor)',
        demo: 'https://elsayed23.github.io/calculator-app/',
        code: 'https://github.com/Elsayed23/calculator-app'
    },
    {
        thumbnailImage: require('../Images/age_calculator.png'),
        title: 'Age calculator (front-end-mentor)',
        demo: 'https://elsayed23.github.io/age_calculator/',
        code: 'https://github.com/Elsayed23/age_calculator'
    },
    {
        thumbnailImage: require('../Images/chat-app.png'),
        title: 'Chat-app (ui only) (front-end-mentor)',
        demo: 'https://elsayed23.github.io/chat-app/',
        code: 'https://github.com/Elsayed23/chat-app'
    },
]



export const featuresData = [
    {
        logoUrl: require('../Images/html5.png'),
        name: 'HTML'
    },
    {
        logoUrl: require('../Images/css3.png'),
        name: 'CSS'
    },
    {
        logoUrl: require('../Images/js.png'),
        name: 'JavaScript'
    },
    {
        logoUrl: <svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="react" className="svg-inline--fa fa-react text-6xl" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            <path fill="#61dbfb" d="M418.2 177.2c-5.4-1.8-10.8-3.5-16.2-5.1.9-3.7 1.7-7.4 2.5-11.1 12.3-59.6 4.2-107.5-23.1-123.3-26.3-15.1-69.2.6-112.6 38.4-4.3 3.7-8.5 7.6-12.5 11.5-2.7-2.6-5.5-5.2-8.3-7.7-45.5-40.4-91.1-57.4-118.4-41.5-26.2 15.2-34 60.3-23 116.7 1.1 5.6 2.3 11.1 3.7 16.7-6.4 1.8-12.7 3.8-18.6 5.9C38.3 196.2 0 225.4 0 255.6c0 31.2 40.8 62.5 96.3 81.5 4.5 1.5 9 3 13.6 4.3-1.5 6-2.8 11.9-4 18-10.5 55.5-2.3 
            99.5 23.9 114.6 27 15.6 72.4-.4 116.6-39.1 3.5-3.1 7-6.3 10.5-9.7 4.4 4.3 9 8.4 13.6 12.4 42.8 36.8 85.1 51.7 111.2 36.6 27-15.6 35.8-62.9 24.4-120.5-.9-4.4-1.9-8.9-3-13.5 3.2-.9 6.3-1.9 9.4-2.9 57.7-19.1 99.5-50 99.5-81.7 0-30.3-39.4-59.7-93.8-78.4zM282.9 92.3c37.2-32.4 71.9-45.1 87.7-36 16.9 9.7 23.4 48.9 12.8 100.4-.7 3.4-1.4 6.7-2.3 10-22.2-5-44.7-8.6-67.3-10.6-13-18.6-27.2-36.4-42.6-53.1 3.9-3.7 7.7-7.2 11.7-10.7zM167.2 307.5c5.1 8.7 10.3 17.4 15.8 25.9-15.6-1.7-31.1-4.2-46.4-7.5
             4.4-14.4 9.9-29.3 16.3-44.5 4.6 8.8 9.3 17.5 14.3 26.1zm-30.3-120.3c14.4-3.2 29.7-5.8 45.6-7.8-5.3 8.3-10.5 16.8-15.4 25.4-4.9 8.5-9.7 17.2-14.2 26-6.3-14.9-11.6-29.5-16-43.6zm27.4 68.9c6.6-13.8 13.8-27.3 21.4-40.6s15.8-26.2 24.4-38.9c15-1.1 30.3-1.7 45.9-1.7s31 .6 45.9 1.7c8.5 12.6 16.6 25.5 24.3 38.7s14.9 26.7 21.7 40.4c-6.7 13.8-13.9 27.4-21.6 40.8-7.6 13.3-15.7 26.2-24.2 39-14.9 1.1-30.4 1.6-46.1 1.6s-30.9-.5-45.6-1.4c-8.7-12.7-16.9-25.7-24.6-39s-14.8-26.8-21.5-40.6zm180.6 51.2c5.1-8.8 9.9-17.7 14.6-26.7 6.4 14.5 12 29.2 16.9 44.3-15.5 3.5-31.2 6.2-47 8 5.4-8.4 10.5-17 15.5-25.6zm14.4-76.5c-4.7-8.8-9.5-17.6-14.5-26.2-4.9-8.5-10-16.9-15.3-25.2 16.1 2 31.5 4.7 45.9 8-4.6 14.8-10 29.2-16.1 43.4zM256.2 118.3c10.5 11.4 20.4 23.4 29.6 35.8-19.8-.9-39.7-.9-59.5 0 9.8-12.9 19.9-24.9 29.9-35.8zM140.2 57c16.8-9.8 54.1 4.2 93.4 39 2.5 2.2 5 4.6 7.6 7-15.5 16.7-29.8 34.5-42.9 53.1-22.6 2-45 5.5-67.2 10.4-1.3-5.1-2.4-10.3-3.5-15.5-9.4-48.4-3.2-84.9 12.6-94zm-24.5 263.6c-4.2-1.2-8.3-2.5-12.4-3.9-21.3-6.7-45.5-17.3-63-31.2-10.1-7-16.9-17.8-18.8-29.9 0-18.3 31.6-41.7 77.2-57.6 5.7-2 11.5-3.8 17.3-5.5 6.8 21.7 15 43 24.5 63.6-9.6 20.9-17.9 42.5-24.8 64.5zm116.6 98c-16.5 15.1-35.6 27.1-56.4 35.3-11.1 5.3-23.9 5.8-35.3 1.3-15.9-9.2-22.5-44.5-13.5-92 1.1-5.6 2.3-11.2 3.7-16.7 22.4 4.8 45 8.1 67.9 9.8 13.2 18.7 27.7 36.6 43.2 53.4-3.2 3.1-6.4 6.1-9.6 8.9zm24.5-24.3c-10.2-11-20.4-23.2-30.3-36.3 9.6.4 19.5.6 29.5.6 10.3 0 20.4-.2 30.4-.7-9.2 12.7-19.1 24.8-29.6 36.4zm130.7 30c-.9 12.2-6.9 23.6-16.5 31.3-15.9 9.2-49.8-2.8-86.4-34.2-4.2-3.6-8.4-7.5-12.7-11.5 15.3-16.9 29.4-34.8 42.2-53.6 22.9-1.9 45.7-5.4 68.2-10.5 1 4.1 1.9 8.2 2.7 12.2 4.9 21.6 5.7 44.1 2.5 66.3zm18.2-107.5c-2.8.9-5.6 1.8-8.5 2.6-7-21.8-15.6-43.1-25.5-63.8 9.6-20.4 17.7-41.4 24.5-62.9 5.2 1.5 10.2 3.1 15 4.7 46.6 16 79.3 39.8 79.3 58 0 19.6-34.9 44.9-84.8 61.4zm-149.7-15c25.3 0 45.8-20.5 45.8-45.8s-20.5-45.8-45.8-45.8c-25.3 0-45.8 20.5-45.8 45.8s20.5 45.8 45.8 45.8z">
            </path>
        </svg>,
        name: 'React.js'
    },
    {
        logoUrl: <svg height="5rem" fill='#fff' viewBox=".5 -.2 1023 1024.1" width="5rem" xmlns="http://www.w3.org/2000/svg"><path d="m478.5.6c-2.2.2-9.2.9-15.5 1.4-145.3 13.1-281.4 91.5-367.6 212-48 67-78.7 143-90.3 223.5-4.1 28.1-4.6 36.4-4.6 74.5s.5 46.4 4.6 74.5c27.8 192.1 164.5 353.5 349.9 413.3 33.2 10.7 68.2 18 108 22.4 15.5 1.7 82.5 1.7 98 0 68.7-7.6 126.9-24.6 184.3-53.9 8.8-4.5 10.5-5.7 9.3-6.7-.8-.6-38.3-50.9-83.3-111.7l-81.8-110.5-102.5-151.7c-56.4-83.4-102.8-151.6-103.2-151.6-.4-.1-.8 67.3-1 149.6-.3 144.1-.4 149.9-2.2 153.3-2.6 4.9-4.6 6.9-8.8 9.1-3.2 1.6-6 1.9-21.1 1.9h-17.3l-4.6-2.9c-3-1.9-5.2-4.4-6.7-7.3l-2.1-4.5.2-200.5.3-200.6 3.1-3.9c1.6-2.1 5-4.8 7.4-6.1 4.1-2 5.7-2.2 23-2.2 20.4 0 23.8.8 29.1 6.6 1.5 1.6 57 85.2 123.4 185.9s157.2 238.2 201.8 305.7l81 122.7 4.1-2.7c36.3-23.6 74.7-57.2 105.1-92.2 64.7-74.3 106.4-164.9 120.4-261.5 4.1-28.1 4.6-36.4 4.6-74.5s-.5-46.4-4.6-74.5c-27.8-192.1-164.5-353.5-349.9-413.3-32.7-10.6-67.5-17.9-106.5-22.3-9.6-1-75.7-2.1-84-1.3zm209.4 309.4c4.8 2.4 8.7 7 10.1 11.8.8 2.6 1 58.2.8 183.5l-.3 179.8-31.7-48.6-31.8-48.6v-130.7c0-84.5.4-132 1-134.3 1.6-5.6 5.1-10 9.9-12.6 4.1-2.1 5.6-2.3 21.3-2.3 14.8 0 17.4.2 20.7 2z" /><path d="m784.3 945.1c-3.5 2.2-4.6 3.7-1.5 2 2.2-1.3 5.8-4 5.2-4.1-.3 0-2 1-3.7 2.1zm-6.9 4.5c-1.8 1.4-1.8 1.5.4.4 1.2-.6 2.2-1.3 2.2-1.5 0-.8-.5-.6-2.6 1.1zm-5 3c-1.8 1.4-1.8 1.5.4.4 1.2-.6 2.2-1.3 2.2-1.5 0-.8-.5-.6-2.6 1.1zm-5 3c-1.8 1.4-1.8 1.5.4.4 1.2-.6 2.2-1.3 2.2-1.5 0-.8-.5-.6-2.6 1.1zm-7.6 4c-3.8 2-3.6 2.8.2.9 1.7-.9 3-1.8 3-2 0-.7-.1-.6-3.2 1.1z" /></svg>,
        name: 'Next.js'
    },
    {
        logoUrl: <svg fill="#06B6D4" strokeWidth="0" role="img" viewBox="0 0 24 24" height="5rem" width="5rem" xmlns="http://www.w3.org/2000/svg"><title></title><path d="M12.001,4.8c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 C13.666,10.618,15.027,12,18.001,12c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C16.337,6.182,14.976,4.8,12.001,4.8z M6.001,12c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 c1.177,1.194,2.538,2.576,5.512,2.576c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C10.337,13.382,8.976,12,6.001,12z"></path></svg>,
        name: 'Tailwind'
    },
    {
        logoUrl: <svg fill="#764abc" strokeWidth="0" role="img" viewBox="0 0 24 24" height="5rem" width="5rem" xmlns="http://www.w3.org/2000/svg"><title></title><path d="M16.634 16.504c.87-.075 1.543-.84 1.5-1.754-.047-.914-.796-1.648-1.709-1.648h-.061a1.71 1.71 0 00-1.648 1.769c.03.479.226.869.494 1.153-1.048 2.038-2.621 3.536-5.005 4.795-1.603.838-3.296 1.154-4.944.93-1.378-.195-2.456-.81-3.116-1.799-.988-1.499-1.078-3.116-.255-4.734.6-1.17 1.499-2.023 2.099-2.443a9.96 9.96 0 01-.42-1.543C-.868 14.408-.416 18.752.932 20.805c1.004 1.498 3.057 2.456 5.304 2.456.6 0 1.23-.044 1.843-.194 3.897-.749 6.848-3.086 8.541-6.532zm5.348-3.746c-2.32-2.728-5.738-4.226-9.634-4.226h-.51c-.253-.554-.837-.899-1.498-.899h-.045c-.943 0-1.678.81-1.647 1.753.03.898.794 1.648 1.708 1.648h.074a1.69 1.69 0 001.499-1.049h.555c2.309 0 4.495.674 6.488 1.992 1.527 1.005 2.622 2.323 3.237 3.897.538 1.288.509 2.547-.045 3.597-.855 1.647-2.294 2.517-4.196 2.517-1.199 0-2.367-.375-2.967-.644-.36.298-.96.793-1.394 1.093 1.318.598 2.652.943 3.94.943 2.922 0 5.094-1.647 5.919-3.236.898-1.798.824-4.824-1.47-7.416zM6.49 17.042c.03.899.793 1.648 1.708 1.648h.06a1.688 1.688 0 001.648-1.768c0-.9-.779-1.647-1.693-1.647h-.06c-.06 0-.15 0-.226.029-1.243-2.098-1.768-4.347-1.572-6.772.12-1.828.72-3.417 1.797-4.735.9-1.124 2.593-1.68 3.747-1.708 3.236-.061 4.585 3.971 4.689 5.574l1.498.45C17.741 3.197 14.686.62 11.764.62 9.02.62 6.49 2.613 5.47 5.535 4.077 9.43 4.991 13.177 6.7 16.174c-.15.195-.24.539-.21.868z"></path></svg>,
        name: 'Redux'
    },
    {
        logoUrl: require('../Images/mySQL.png'),
        name: 'MySQL'
    },
    {
        logoUrl: require('../Images/node.png'),
        name: 'Node.js'
    },
    {
        logoUrl: require('../Images/express.png'),
        name: 'Express.js'
    },
    {
        logoUrl: <svg fill="none" height="5rem" width="9rem" viewBox="1.372 -.18543865 324.553 128.18543865" xmlns="http://www.w3.org/2000/svg">
            <g fill="#0c344b"><path d="m199.202 85.75h8.638v-31.662h-8.638zm-.367-39.847c0-2.813 1.567-4.219 4.701-4.219 3.133 0 4.701 1.406 4.701 4.219 0 1.341-.392 2.384-1.175 3.13-.784.746-1.959 1.118-3.526 1.118-3.134 0-4.701-1.416-4.701-4.248z" />
                <path clipRule="evenodd" d="m164.253 67.483c2.786-2.36 4.178-5.767 4.178-10.223 0-4.286-1.307-7.51-3.922-9.672-2.615-2.16-6.433-3.242-11.456-3.242h-13.225v41.404h8.779v-14.727h3.767c5.135 0 9.095-1.179 11.879-3.54zm-12.757-3.653h-2.889v-12.29h3.993c2.398 0 4.158.49 5.282 1.472 1.123.982 1.685 2.502 1.685 4.56 0 2.038-.67 3.591-2.011 4.658s-3.36 1.6-6.06 1.6z" fillRule="evenodd" />
                <path d="m194.62 53.748c-.774-.17-1.746-.255-2.917-.255-1.964 0-3.781.543-5.451 1.628a11.908 11.908 0 0 0 -3.98 4.291h-.424l-1.275-5.324h-6.542v31.662h8.638v-16.114c0-2.549.769-4.532 2.307-5.948 1.54-1.416 3.687-2.124 6.444-2.124 1.001 0 1.85.095 2.549.283zm40.245 30.02c2.257-1.7 3.385-4.172 3.385-7.42 0-1.567-.273-2.917-.821-4.05-.547-1.133-1.398-2.133-2.549-3.002-1.151-.868-2.964-1.802-5.438-2.803-2.775-1.114-4.573-1.955-5.394-2.521s-1.233-1.236-1.233-2.011c0-1.378 1.275-2.067 3.824-2.067 1.434 0 2.841.217 4.219.65 1.378.436 2.861.992 4.447 1.672l2.605-6.23c-3.606-1.661-7.316-2.492-11.13-2.492-4.003 0-7.093.769-9.273 2.308-2.183 1.539-3.273 3.714-3.273 6.527 0 1.643.26 3.026.78 4.149.518 1.124 1.349 2.12 2.493 2.988 1.14.869 2.931 1.813 5.365 2.832 1.699.718 3.059 1.345 4.079 1.883 1.019.539 1.737 1.02 2.153 1.445.415.425.622.977.622 1.657 0 1.812-1.567 2.718-4.702 2.718-1.529 0-3.299-.255-5.309-.764-2.012-.51-3.819-1.142-5.424-1.898v7.137a22.275 22.275 0 0 0 4.56 1.373c1.624.312 3.587.468 5.891.468 4.492 0 7.867-.85 10.123-2.55zm37.604 1.982h-8.638v-18.493c0-2.284-.383-3.998-1.146-5.14-.766-1.142-1.969-1.714-3.612-1.714-2.208 0-3.813.812-4.814 2.436s-1.501 4.295-1.501 8.015v14.896h-8.638v-31.662h6.599l1.161 4.05h.482c.849-1.454 2.077-2.592 3.681-3.413 1.605-.821 3.446-1.232 5.523-1.232 4.739 0 7.948 1.549 9.629 4.645h.764c.85-1.473 2.101-2.615 3.753-3.427s3.516-1.218 5.593-1.218c3.587 0 6.302.921 8.142 2.761 1.841 1.841 2.761 4.791 2.761 8.85v20.646h-8.666v-18.493c0-2.284-.383-3.998-1.146-5.14-.766-1.142-1.969-1.714-3.612-1.714-2.114 0-3.695.756-4.744 2.266-1.047 1.511-1.571 3.908-1.571 7.193z" />
                <path clipRule="evenodd" d="m318.222 81.445 1.671 4.305h6.032v-21.099c0-3.776-1.133-6.589-3.398-8.439-2.266-1.85-5.523-2.776-9.771-2.776-4.436 0-8.477.954-12.121 2.861l2.86 5.834c3.417-1.53 6.391-2.294 8.921-2.294 3.285 0 4.928 1.605 4.928 4.814v1.388l-5.494.17c-4.739.17-8.283 1.053-10.635 2.648-2.35 1.596-3.525 4.074-3.525 7.434 0 3.21.873 5.683 2.619 7.42 1.747 1.737 4.139 2.605 7.18 2.605 2.473 0 4.479-.354 6.017-1.062 1.539-.708 3.035-1.977 4.489-3.809zm-4.22-10.252 3.342-.113v2.605c0 1.908-.6 3.437-1.799 4.588-1.198 1.152-2.799 1.728-4.8 1.728-2.794 0-4.191-1.218-4.191-3.653 0-1.7.613-2.964 1.841-3.795 1.227-.83 3.096-1.284 5.607-1.36zm-218.269 30.336-57.479 17c-1.756.52-3.439-.999-3.07-2.77l20.534-98.34c.384-1.838 2.926-2.13 3.728-.427l38.02 80.736c.717 1.523-.101 3.319-1.733 3.801zm9.857-4.01-44.022-93.482v-.002a7.062 7.062 0 0 0 -6.019-4.022c-2.679-.156-5.079 1.136-6.433 3.335l-47.744 77.33a7.233 7.233 0 0 0 .084 7.763l23.338 36.152c1.391 2.158 3.801 3.407 6.306 3.407.71 0 1.424-.1 2.126-.308l67.744-20.036a7.424 7.424 0 0 0 4.66-4.028 7.264 7.264 0 0 0 -.04-6.11z" fillRule="evenodd" /></g></svg>,
        name: 'Prisma ORM'
    },
    {
        logoUrl: API,
        name: 'Rest-API'
    },
    {
        logoUrl: require('../Images/clean-code.png'),
        name: 'Clean Code'
    },
    {
        logoUrl: require('../Images/react-router.png'),
        name: 'React router dom'
    },
    {
        logoUrl: <svg fill="#f1502f" strokeWidth="0" viewBox="0 0 448 512" height="5rem" width="5rem" xmlns="http://www.w3.org/2000/svg"><path d="M439.55 236.05L244 40.45a28.87 28.87 0 0 0-40.81 0l-40.66 40.63 51.52 51.52c27.06-9.14 52.68 16.77 43.39 43.68l49.66 49.66c34.23-11.8 61.18 31 35.47 56.69-26.49 26.49-70.21-2.87-56-37.34L240.22 199v121.85c25.3 12.54 22.26 41.85 9.08 55a34.34 34.34 0 0 1-48.55 0c-17.57-17.6-11.07-46.91 11.25-56v-123c-20.8-8.51-24.6-30.74-18.64-45L142.57 101 8.45 235.14a28.86 28.86 0 0 0 0 40.81l195.61 195.6a28.86 28.86 0 0 0 40.8 0l194.69-194.69a28.86 28.86 0 0 0 0-40.81z"></path></svg>,
        name: 'Git'
    },
    {
        logoUrl: require('../Images/github.png'),
        name: 'Github'
    },
]